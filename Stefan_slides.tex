	...
	\begin{frame}[fragile]
		...
		\begin{columns}
			...
			\begin{center}
				...
				\includegraphics[width=0.95 \textwidth]{IMG2/InternalDACsFSM.pdf}
			\end{center}
			\column{0.4 \textwidth}
			\begin{itemize}
...
%	\end{frame}
	...
	\begin{frame}
	\frametitle{2=>Reading Internal DACs for threshold-tuning}
	\begin{columns}
		\column{0.5 \textwidth}
		{\color{blue} Input}
		\begin{itemize}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item I implemented a \textbf{64 bit} counter running at 500 MHz and the logic to read it
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{frame}
		...
		\begin{columns}
			...
			\column{0.3 \textwidth}
			\begin{itemize}
				\item 1~MHz pulsed rate
				\item 100~ms integration time
				\item measurement from \textbf{ch~16}
				\item $\sigma_{CPU}$ = {\color{red}341~Hz} 
				\item $\sigma_{FPGA}$ = {\color{blue}4~Hz}
			\end{itemize}
		\end{columns}
		{\color{blue} Same measurements were performed with \textbf{200~ms}, \textbf{300~ms} ans \textbf{500~ms} integration time:\\average \textbf{90~times} $\sigma$ improvement}
	\end{frame}
