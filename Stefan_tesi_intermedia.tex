...
\begin{frame}
	\frametitle{FPGA function}
	\begin{center}
		{\huge What does the \textbf{FPGA} board do?}
		\begin{itemize}
			\item Receives the \textbf{outputs} from the abacus chip
			\item For each channel counts the number of \textbf{pulses} and the number of clock strokes with high signal.
			\item Sends the data to a \textbf{PC} via UDP protocol
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{columns}
		...
		\end{center}
		\begin{itemize}
			\item ABACUS DAC I/O = \textbf{1.2}V single-ended
			\item FPGA I/O = \textbf{2.5}V single-ended
